...
function createImage(template,source,x,y,w,h){
  ...
  ctx.fillStyle="#ffc442";
  ctx.fill();
  ctx.drawImage(userimage,x,y,w,h);
  ctx.drawImage(cover,0,0,500,500);

  var base64 = resize_canvas.toDataURL("application/octet-stream");
  // $("#smallhide").attr("src",base64);
  // $("#ik").html($('#smallhide').attr("src"));
...
function loadImage(files) {
  ...
  function imageLoaded() {
    ...
    var base64 = canvas.toDataURL("application/octet-stream");
    ...
    var thumbbase64 = thumb.toDataURL("application/octet-stream");
